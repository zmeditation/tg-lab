    ...
    setTeamScores: (state: toumamantState, data: any) => {
      ...
      for (let i = 0; i < keys.length; i++) {
        for (let j = i + 1; j < keys.length; j++) {
          if (played[+keys[i].split("-")[0]] === undefined)
            played[+keys[i].split("-")[0]] = 0;
          if (played[+keys[i].split("-")[1]] === undefined)
            played[+keys[i].split("-")[1]] = 0;
          if (draw[+keys[i].split("-")[0]] === undefined)
            draw[+keys[i].split("-")[0]] = 0;
          if (draw[+keys[i].split("-")[1]] === undefined)
            draw[+keys[i].split("-")[1]] = 0;
          if (lost[+keys[i].split("-")[0]] === undefined)
            lost[+keys[i].split("-")[0]] = 0;
          if (lost[+keys[i].split("-")[1]] === undefined)
            lost[+keys[i].split("-")[1]] = 0;
          if (win[+keys[i].split("-")[0]] === undefined)
            win[+keys[i].split("-")[0]] = 0;
          if (win[+keys[i].split("-")[1]] === undefined)
            win[+keys[i].split("-")[1]] = 0;
          if (points[+keys[i].split("-")[0]] === undefined)
            points[+keys[i].split("-")[0]] = 0;
          if (points[+keys[i].split("-")[1]] === undefined)
            points[+keys[i].split("-")[1]] = 0;
          if (
            keys[i].split("-")[0] === keys[j].split("-")[0] &&
            keys[i].split("-")[1] === keys[j].split("-")[1]
          ) {
        ...
        state.teams[+keys[i].split("-")[0]].played =
          played[+keys[i].split("-")[0]];
        state.teams[+keys[i].split("-")[1]].played =
          played[+keys[i].split("-")[1]];
        ...
        state.teams[+keys[i].split("-")[0]].points =
          draw[+keys[i].split("-")[0]] + 3 * win[+keys[i].split("-")[0]];
        state.teams[+keys[i].split("-")[1]].points =
          draw[+keys[i].split("-")[1]] + 3 * win[+keys[i].split("-")[1]];
      }
      localStorage.setItem("teamInfo", JSON.stringify(state.teams));
      state.teams = JSON.parse(localStorage.getItem("teamInfo") || "{}");
    },
...
export const { saveTeamInfo, saveTeamScore, setTeamScores } =
  toumamentReducer.actions;
